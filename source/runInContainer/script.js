...
function unitTest(input) // When running inside container, docker client communicates with MobeyLinuxVM on Windows host machine, and the volume paths will be related or referencing to the hyper-v MobyLinuxVM vm. In it here is a folder /host_mount/c that corresponds to the Widnows host filesystem drive.
// when variable names are similar declaring the variable and assigning it in the first object causes not defined ReferenceError.
{
  ...
  ({
    ...
    // path of the module that includes the test framework.
    testPath = configuration.directory.testPath,
    // path to test directory.
    applicationPathOnHostMachine = _path.default.join(configuration.directory.projectPath, 'application') // this path should be already resolved to Unix path from Windows path including the drive letter, which will be used in MobyLinuxVM.
  ...
  let processCommand = 'docker-compose',
    ...
    env: Object.assign(process.env, // PATH environment variable is required for docker-composer to run.  PATH - specifies the directories in which executable programs
    environmentVariable)
  };
