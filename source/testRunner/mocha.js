...
function runMocha({
  mocha = new _mocha.default({
    ui: 'tdd',
    // Note: not using https://mochajs.org/#require interface because it doesn't work with node cli, it requires running tests through `mocha` cli as mentioned in https://github.com/mochajs/mocha/issues/1160
    reporter: 'progress' || 'min'
    /*min removes any console.log output outside of test/it blocks*/
    // https://mochajs.org/#list
  ...
  if (shouldInvalidateRequireModule) {
    const {
      invalidateRequiredModule,
      invalidateRequiredModuleEventHandler
    } = './utility/invalidateRequiredModule.js';
    invalidateRequiredModuleEventHandler({
      mochaInstance: mocha
    });
    invalidateRequiredModule({
  ...
  try {
    mocha.run(error => {
      ...
      if (error) {} // mocha handles printing error message.
      // throw error
      // process.exit()
